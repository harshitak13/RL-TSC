# =============================================================
# colight_agent.py  —  fully self-contained CoLight DQN agent
# =============================================================
...
# Use gymnasium if available, fall back to gym
try:
...
# ---------------------------------------------------------------------------
# Safely detect whether a *working* torch_geometric + torch_scatter is present.
# Both libraries may be installed but have broken .pyd binaries on Windows —
# we catch ANY exception during import so the fallback always triggers.
# ---------------------------------------------------------------------------
_HAS_GEO = False
try:
    # Suppress the UserWarning that torch_geometric itself emits on bad installs
    import warnings
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        from torch_geometric.nn import MessagePassing
        from torch_geometric.utils import add_self_loops
        import torch_scatter                          # this is the one that crashes
    _HAS_GEO = True
except Exception:
    _HAS_GEO = False

if _HAS_GEO:
    print("[CoLightAgent] torch_geometric + torch_scatter OK — using real GAT.")
else:
    print("[CoLightAgent] torch_geometric/torch_scatter unavailable — using MLP fallback.")


# ---------------------------------------------------------------
# Attention block  (real GAT or MLP fallback)
# ---------------------------------------------------------------
if _HAS_GEO:
    ...
    class _AttBlock(nn.Module):
        """MLP fallback — same interface as the real GAT block."""
        def __init__(self, d, dv, d_out, nv):
            super().__init__()
...
class CoLightAgent:
    def __init__(self, world, vehicle_max=20, gamma=0.99, lr=1e-3,
                 ...
                 batch_size=32, buffer_size=5000, grad_clip=1.0):
        ...
        self.sub_agents    = len(world.intersections)

        if self.sub_agents == 1:
            self.edge_idx = torch.tensor([[0], [0]], dtype=torch.long)
        else:
            self.edge_idx = torch.tensor(
                world.graph['sparse_adj'].T, dtype=torch.long)
    ...
    def get_action(self, obs: np.ndarray, test: bool = False) -> np.ndarray:
        ...
        with torch.no_grad():
            q = self.model(x, self.edge_idx)
        return np.array([int(torch.argmax(q[i, :pl]).item())
                         for i, pl in enumerate(self.phase_lengths)])

    # --- store ---------------------------------------------------------
    def store(self, obs, next_obs, reward, action):
        self.replay_buffer.append((obs, next_obs, float(reward), action))
    ...
    def train(self) -> float:
        ...
        a  = torch.tensor(a,            dtype=torch.long).view(-1)
